...
function sel_province(id_province) {
    ...
    $('#ddl_province').prop('disabled', $('#ddl_province option').length <= 1);

}

function sel_city(id_city) {
    $('#ddl_city').val(id_city);
    $('#ddl_city').prop('disabled', $('#ddl_city option').length <= 1); /*WAS LENGTH == 0*/

    if (typeof id_city == 'undefined' && $('#ddl_city option').length > 1) {
        $('#ddl_city').val(-1);
...
$(document)
    ...
    .on('change', '#ddl_country', function () {
        change_country();
    })

    .on('change', '#ddl_province', function () {
        change_province();
    });
